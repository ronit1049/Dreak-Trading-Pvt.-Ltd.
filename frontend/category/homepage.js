 ...
 *   1. Cart badge — reads liventra_cart from localStorage and
...
function syncCartBadge() {
    try {
        const cart = JSON.parse(localStorage.getItem("liventra_cart")) || [];
...
function buildProductCard(product) {
    ...
    const imgHtml = img
        ? `<img src="${BASE_URL}${esc(img)}" alt="${esc(product.name)}" loading="lazy" class="prod-img" />`
    ...
    return `
        <a class="fp-card" href="/product?slug=${encodeURIComponent(product.slug)}"
...
async function loadProducts(categorySlug) {
    ...
    try {
        ...
        console.warn("[Liventra] Featured products failed:", err);
        grid.innerHTML = `
            <div class="fp-empty">
...
async function buildFeaturedTabs() {
    ...
    try {
        ...
        console.warn("[Liventra] Featured categories failed:", err);
